...
function Login(){
  const [form,setForm]=useState({email:'',password:''}); const [error,setError]=useState(''); const [isSubmitting,setIsSubmitting]=useState(false); const [showPassword,setShowPassword]=useState(false); const [verificationEmail,setVerificationEmail]=useState(''); const navigate=useNavigate(); const dispatch=useDispatch();
  const googleEnabled = Boolean(import.meta.env.VITE_GOOGLE_CLIENT_ID);
  const finishAuthentication=async(token)=>{localStorage.setItem('token',token);const userRes=await api.get('/users/me');dispatch(setCredentials({user:userRes.data,token}));navigate('/dashboard');};
  const handleSubmit=async(event)=>{event.preventDefault();setError('');setIsSubmitting(true);try{const res=await api.post('/auth/login',form);const token=res.data.token;localStorage.setItem('token',token);const userRes=await api.get('/users/me');dispatch(setCredentials({user:userRes.data,token}));navigate('/dashboard');}catch(err){if(err.response?.data?.requiresVerification)setVerificationEmail(err.response.data.email||form.email);else setError(err.response?.data?.message||'Login failed');}finally{setIsSubmitting(false);}};
