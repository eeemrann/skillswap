...
function Login(){
  ...
  const handleSubmit=async(event)=>{event.preventDefault();setError('');setIsSubmitting(true);try{const res=await api.post('/auth/login',form);const token=res.data.token;sessionStorage.setItem('token',token);const userRes=await api.get('/users/me');dispatch(setCredentials({user:userRes.data,token}));navigate('/dashboard');}catch(err){sessionStorage.removeItem('token');if(err.response?.data?.requiresVerification)setVerificationEmail(err.response.data.email||form.email);else setError(err.response?.data?.message||'Login failed');}finally{setIsSubmitting(false);}};
