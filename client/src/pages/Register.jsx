...
function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data.requiresVerification) { setVerificationMessage(response.data.message); setVerificationEmail(response.data.email); }
      else navigate('/login');
  ...
  return <AuthLayout mode="register">
    {verificationEmail
      ? <EmailVerificationForm email={verificationEmail} initialMessage={verificationMessage} onVerified={() => navigate('/login')} onBack={() => setVerificationEmail('')} />
      : <>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <div className="form-field"><label htmlFor="email">Email address</label><input id="email" name="email" type="email" autoComplete="email" placeholder="you@example.com" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required /><span className="form-hint">You will need to verify this address before signing in.</span></div>
